...
function OnLoadCollabAddressBook () {
  ...
  if (tabcollab) {
    tabcollab.registerTabType(contactsTabType);
    tabcollab.openTab("contacts", {background: true});
  }

  if (typeof(chatController) != "undefined") {
    addCommonController(chatController);
    chatController.updateCommands();
  }
  ...
  modes: {
    contacts: {
      ...
      openTab: function(aTab, aArgs) {
        ...
        aTab.title = StringBundle.getString("contactsTitle");
      },

      showTab: function(aTab) {
        var focusedElement = aTab.focusedElement;
        if (!focusedElement)
          focusedElement = document.getElementById("abResultsTree");
