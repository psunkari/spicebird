  ...
  onStateChange: function tPL_onStateChange(aWebProgress, aRequest, aStateFlags,
    ...
    if (aStateFlags & nsIWebProgressListener.STATE_START &&
      ...
      if (!this.mBlank) {
        ...
        tabcollab.setTabBusy(this.mTab, true);
        tabcollab.setTabtitle(this.mTab);
      }
    }
    else if (aStateFlags & nsIWebProgressListener.STATE_STOP &&
             aStateFlags & nsIWebProgressListener.STATE_IS_NETWORK) {
      ...
      tabcollab.setTabBusy(this.mTab, false);
    }
  },
...
var specialTabs = {
  ...
  openSpecialTabsOnStartup: function() {
    ...
    tabcollab.registerTabType(this.contentTabType);
    tabcollab.registerTabType(this.chromeTabType);
  ...
  contentTabType: {
    ...
    restoreTab: function onRestoreTab(aTabCollab, aPersistedState) {
      aTabCollab.openTab("contentTab", { contentPage: aPersistedState.tabURI,
    ...
    _setUpTitleListener: function setUpTitleListener(aTab) {
      function onDOMTitleChanged(aEvent) {
        document.getElementById("tabcollab").setTabTitle(aTab);
      }
    ...
    _setUpCloseWindowListener: function setUpCloseWindowListener(aTab) {
      function onDOMWindowClose(aEvent) {
        ...
        document.getElementById("tabcollab").closeTab(aTab);
  ...
  getApplicationUpgradeVersions: function(prefs) {
    ...
    let prefstring = "collab.start_page_override.mstone";
  ...
  shouldShowAboutRightsNotification: function(prefs) {
    /*
     * Spicebird does not currently have a about:rights page
     */
    return false;

    try {
      return !prefs.getBoolPref("collab.rights.override");
    } catch (e) { }

    return prefs.getIntPref("collab.rights.version") < this._kAboutRightsVersion;
  },

  showAboutRightsNotification: function(prefs) {
    var notifyBox = document.getElementById("notification-box");
    ...
    prefs.setIntPref("collab.rights.version", this._kAboutRightsVersion);
  },
  ...
  chromeTabType: {
    ...
    restoreTab: function onRestoreTab(aTabCollab, aPersistedState) {
      aTabCollab.openTab("chromeTab", { chromePage: aPersistedState.tabURI,
                                      clickHandler: aPersistedState.clickHandler,
                                      background: true } );
    },
    onTitleChanged: function onTitleChanged(aTab) {
