...
LOCALES_collab :=                               \
  ...
  calendar                                      \
  collab                                        \
  collab/extensions/google-account              \
  collab/extensions/google-applets              \
  $(NULL)
...
MOZ_CO_LOCALES_DATE  = 25 Jul 2008 00:00 PDT
...
COLLAB_CVSROOT ?= https://svn.spicebird.org/repos/collab/trunk

CHECKOUT_COLLAB = svn -q checkout $(COLLAB_CVSROOT) mozilla/collab
CHECKOUT_COLLAB_ALL_LOCALES = $(CVS) $(CVS_FLAGS) co mozilla/tools/l10n; svn -q checkout $(COLLAB_CVSROOT)/locales/ mozilla/collab/locales
PATCH_MOZILLA = cd mozilla && collab/patches/patch.sh && cd ..
...
CHECKOUT_MODULES   := cvs_co $(CVS) $(CVS_FLAGS) co $(MODULES_CO_FLAGS) $(CVS_CO_DATE_FLAGS) $(filter-out mozilla/collab,$(MOZ_MODULE_LIST));
endif
...
CHECKOUT_LOCALES_COLLAB := true
PATCH_MOZILLA_L10N := true
else

override MOZ_CO_LOCALES := $(subst $(comma), ,$(MOZ_CO_LOCALES))

ifeq (all,$(MOZ_CO_LOCALES))
MOZCONFIG_MODULES += $(foreach project,$(filter-out collab,$(MOZ_PROJECT_LIST)),mozilla/$(project)/locales/all-locales)

LOCALE_CO_DIRS = $(sort $(foreach project,$(MOZ_PROJECT_LIST),$(foreach locale,$(shell cat $(ROOTDIR)/mozilla/$(project)/locales/all-locales),l10n/$(locale)/)))

else # MOZ_CO_LOCALES != all
LOCALE_CO_DIRS = $(sort $(foreach locale,$(MOZ_CO_LOCALES),l10n/$(locale)/))
endif

CVSCO_LOCALES = for LOCALE_CO_DIR in $(LOCALE_CO_DIRS) ; do cvs_co $(CVS) $(CVS_FLAGS) -d $(LOCALES_CVSROOT) co $(LOCALES_CO_FLAGS) $(CVS_CO_LOCALES_DATE_FLAGS) $$LOCALE_CO_DIR ; done

FASTUPDATE_LOCALES := fast_update $(CVSCO_LOCALES)
CHECKOUT_LOCALES := $(CVSCO_LOCALES)

COLLAB_L10N_CVSROOT ?= https://svn.spicebird.org/repos/l10n/trunk
ifeq (all,$(MOZ_CO_LOCALES))
MOZ_COLLAB_CO_LOCALES = $(sort $(shell cat $(ROOTDIR)/mozilla/collab/locales/all-locales))
else
MOZ_COLLAB_CO_LOCALES = $(MOZ_CO_LOCALES)
endif
CHECKOUT_LOCALES_COLLAB = for LOCALE in $(MOZ_COLLAB_CO_LOCALES) ; do cvs_co svn -q checkout $(COLLAB_L10N_CVSROOT)/$$LOCALE l10n_collab/$$LOCALE; done
PATCH_MOZILLA_L10N = cd $(TOPSRCDIR) && make -f tools/l10n/l10n.mk create-xx-yy; cd $(ROOTDIR) && mkdir -p l10n_stage1 l10n_full; for LOCALE in $(MOZ_COLLAB_CO_LOCALES) ; do cvs_co mozilla/collab/tools/l10n/merge_locale_directories.pl l10n/xx-yy l10n/$$LOCALE l10n_stage1/$$LOCALE; cvs_co mozilla/collab/tools/l10n/merge_locale_directories.pl l10n_stage1/$$LOCALE l10n_collab/$$LOCALE l10n_full/$$LOCALE; done ; mv l10n l10n_upstream; mv l10n_full l10n;
...
ifdef RUN_AUTOCONF_LOCALLY
	...
	cd $(ROOTDIR) && $(CHECKOUT_COLLAB_ALL_LOCALES)
...
real_checkout:
	...
	$(CHECKOUT_MODULES_NS); \
	$(CHECKOUT_MODULES) \
	cvs_co $(CHECKOUT_COLLAB);
	-$(PATCH_MOZILLA) 2>&1 | tee -a $(CVSCO_LOGFILE);
	@set -e; \
	cvs_co() { set -e; echo "$$@" ; \
	  "$$@" 2>&1 | tee -a $(CVSCO_LOGFILE); }; \
	$(CHECKOUT_LOCALES); \
	$(CHECKOUT_LOCALES_COLLAB); \
	$(PATCH_MOZILLA_L10N)
...
FULL_EN_US_DIRS := $(subst collab, mailnews mail calendar, $(FULL_EN_US_DIRS))

LOCALE_DIRS := $(filter-out collab%,$(LOCALE_DIRS))

EN_US_LOCALE_DIRS := $(foreach dir, \
	$(filter-out toolkit extensions/% $(MOZ_PROJECT_LIST) mailnews mail calendar, $(LOCALE_DIRS)), \
	mozilla/$(dir)/locales)
