...
patch -p1 -N -R < $CURRENT_DIR/$SCRIPT_DIR/mozilla/441414_multiline_trees.patch

# 250. Add more smileys to mozITXTToHTMLConv
patch -p1 -N -R < $CURRENT_DIR/$SCRIPT_DIR/mozilla/streamconv_more_smileys.patch

# 190. Patch to use applications.manifest in profile directory for chome loading
patch -p1 -N -R < $CURRENT_DIR/$SCRIPT_DIR/mozilla/use_applications_manifest_from_profile.patch

# 60. Patch to integrate glib event loop into Windows event loop
patch -p1 -N -R < $CURRENT_DIR/$SCRIPT_DIR/mozilla/windows_glib_event_loop.patch
