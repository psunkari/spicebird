...
x509_ca_init(void)
{
	...
	if (NULL == x509_ca_paths) {
        /* BEGIN SPICEBIRD CHANGES */
                x509_ca_paths = g_list_append(NULL, g_build_filename(
                                              g_getenv("MOZILLA_FIVE_HOME"),
	...
	if (flags & PURPLE_CERTIFICATE_FATALS_MASK) {
		...
		while (i != PURPLE_CERTIFICATE_LAST) {
			...
			i <<= 1;
                ...
                /* BEGIN SPICEBIRD CHANGES */
                /* XXX: Accept all certificates with temporary errors */
                purple_debug_info("spicebird", "%s\n", errors->str);
		purple_certificate_verify_complete(vrq,
						   PURPLE_CERTIFICATE_VALID);
#if 0
