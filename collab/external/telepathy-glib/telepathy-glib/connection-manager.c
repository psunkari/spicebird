...
tp_connection_manager_find_manager_file (const gchar *name)
{
  ...
  filename = g_strdup_printf ("%s/telepathy/managers/%s.manager",
      g_getenv ("MOZILLA_FIVE_HOME"), name);

  DEBUG ("in MOZILLA_FIVE_HOME: trying %s", filename);

#else
  {
    gchar path[MAXPATHLEN] = "";
    gint path_length = sizeof(path);
    gchar * file = g_strconcat (name, ".manager", NULL);

    if (GetModuleFileName (NULL, path, path_length) > 0)
      {
        gchar * sep;
        sep = strrchr (path, '\\');
        if (sep)
    ...
    filename = g_build_filename (path, "telepathy", "managers", file, NULL);

    g_free (file);
  }

  DEBUG ("in application path: trying %s", filename);
