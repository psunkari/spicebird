...
/*
 * This is to memorize who made the first connection to the daemon so that
 * when this connection closes, we exit dbus
 */
static DBusConnection * first_client_connection;
...
bus_connection_disconnected (DBusConnection *connection)
{
  ...
  /* BEGIN SPICEBIRD CHANGES */
  /* 
   * Our parent the spawner has closed connection. There is not other process
   * that knows our address and we have no reason to exist. So die.
   */
  if (first_client_connection == connection)
      _dbus_loop_quit (connection_get_loop (connection));
  /* END SPICEBIRD CHANGES */
                                  ...
                                  DBusConnection *connection)
{
  ...
  /* BEGIN SPICEBIRD CHANGES */
  /* 
   * If this is the first connection we note it down assuming that it is our
   * spawner parent that always makes the first connection.
   */
  if (!first_client_connection)
      first_client_connection = connection;
  /* END SPICEBIRD CHANGES */
