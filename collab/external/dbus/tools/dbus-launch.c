...
main (int argc, char **argv)
{
  ...
  if (autolaunch)
    {      
      ...
      /* Our dbus daemon was patched so that it will exit as soon as the its
       * first client is gone. It does not need a babysitter and x11
       * monitoring. x11 support will not be compiled in.
       */
#if 0
...
#else
      {
      char * daemon_dir = getenv("MOZILLA_FIVE_HOME");
      char * daemon_name = "/dbus-daemon";
      char * daemon_path = (char *) malloc (strlen(daemon_dir) + 
                                            strlen(daemon_name) + 1);
      if (daemon_path == NULL)
        {
          fprintf (stderr, "Out of memory!\n");
          exit (1); 
        }
      strcpy(daemon_path, daemon_dir);
