...
#else
  {
  char * bin_dir = getenv ("MOZILLA_FIVE_HOME");
  char * bin_file = "/dbus-launch";
  char * bin_path = malloc ((strlen (bin_dir) + strlen(bin_file) + 1) 
                            * sizeof (char));
  if (bin_path == NULL)
    {
      _dbus_verbose("Failed to launch daemon: %s\n",
                    _dbus_strerror(DBUS_ERROR_NO_MEMORY));
      _exit (1);
    }
  strcpy (bin_path, bin_dir);
  strcat (bin_path, bin_file);

  retval = _read_subprocess_line_argv (bin_path,
                                       TRUE,
                                       argv, address, error);
  free (bin_path);
  }
...
_dbus_get_standard_session_servicedirs (DBusList **dirs)
{
  ...
  if (!_dbus_string_append (&servicedir_path, DBUS_DATADIR":"))
        goto oom;
#else
  if (!_dbus_string_append (&servicedir_path, getenv("MOZILLA_FIVE_HOME")) ||
...
#else
  _dbus_string_append (str, getenv ("MOZILLA_FIVE_HOME"));
  return _dbus_string_append (str, "/dbus-1/system.conf");
#endif
  /* END SPICEBIRD CHANGES */
}
...
#else
  _dbus_string_append (str, getenv ("MOZILLA_FIVE_HOME"));
  return _dbus_string_append (str, "/dbus-1/session.conf");
#endif
  /* END SPICEBIRD CHANGES */
}
