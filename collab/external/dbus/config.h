...
/* Don't Build X11-dependent code 
 * For Spicebird, we don't want to rely on x11 session closing
 * for dbus shutdown. Instead we want to rely on first client
 * getting shutdown. So we disable x11 session monitoring 
 * altogether and patch dbus accordingly.
 */
