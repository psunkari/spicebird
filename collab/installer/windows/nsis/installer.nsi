...
#  Prasad Sunkari <prasad@synovel.com> (Spiceird Port)
#
...
Section "-Application" APP_IDX
  ...
  ; On install always add the Spicebird.EML, Spicebird.MAILTO,
  ; Spicebird.NEWS and Calendar.ICS
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $0 "SOFTWARE\Classes"
  StrCpy $1 "$\"$8$\" $\"%1$\""
  StrCpy $2 "$\"$8$\" -osint -compose $\"%1$\""
  StrCpy $3 "$\"$8$\" -osint -mail $\"%1$\""
  StrCpy $4 "$\"$8$\" -osint -import-events $\"%1$\""

  ; An empty string is used for the 5th param because Spicebird.EML
  ; is not a protocol handler
  ${AddHandlerValues} "$0\Spicebird.EML"  "$1" "$8,0" \
                      "${AppRegNameMail} Document" "" ""
  ${AddHandlerValues} "$0\Spicebird.MAILTO"  "$2" "$8,0" \
                      "${AppRegNameMail} URL" "true" ""
  ${AddHandlerValues} "$0\Spicebird.NEWS" "$3" "$8,0" \
                      "${AppRegNameNews} URL" "true" ""
  ${AddHandlerValues} "$0\Spicebird.ICS" "$4" "$8,0" \
