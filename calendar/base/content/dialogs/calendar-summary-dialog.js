...
function onAccept() {
    ...
    if (window.savedParticipationStatus != window.attendee.participationStatus) {
      if (attendee.participationStatus == "ACCEPTED")
...
function updateInvitationStatus() {
    if (!window.readOnly) {
        if (window.attendee) {
            ...
            window.savedParticipationStatus = statusElement.value;
        }
    }
}
