...
function getCurrentUnifinderFilter() {
    let filter = document.getElementById("event-filter-menulist");
    if (filter)
        return filter.selectedItem.value;
...
function prepareCalendarUnifinder() {
    ...
    if (unifinderTree) {
        ...
        unifinderTree.controllers.appendController(UnifinderTreeController);
    }
}
 ...
 * added.
 */
function finishCalendarUnifinder(aEvent) {
    // Check if its really the window that is getting unloaded
    if (aEvent.target != window.document)
      return;
 ...
 * or when the user selects a filter for events
 */
function doUnifinderSearch() {
  if (isUnifinderHidden())
    toggleUnifinder();
...
function addItemsFromCompositeCalendarInternal(eventArray) {
    ...
    /*
     * Temporary fix for Bug 1256
     * If not done, rowCountChanged triggers unifinderSelect 
     * which moves focus away from unifinder-search-field
     */
    var unifinderTree = document.getElementById("unifinder-search-results-tree");
    unifinderTree.onselect = null;
    unifinderTree.removeEventListener("select", unifinderSelect, true);
    unifinderTreeView.selection.selectEventsSuppressed = true;
    unifinderTreeView.selection.clearSelection();
    ...
    /* Part of temporary fix for Bug 1256 above */
    setTimeout(function() { unifinderTreeView.resetAllowSelection(); }, 1);
}
...
function toggleUnifinder() {
    ...
    var filter = document.getElementById("event-filter-menulist");

    // When the unifinder is hidden, refreshEventTree is not called.
    // Make sure the event tree is refreshed now. // Spicebird
    if (!isUnifinderHidden()) {
        refreshEventTree();
        if (filter)
            filter.removeAttribute("disabled");
    } else {
        var searchBar = document.getElementById("unifinder-search-field");
        if(searchBar)
            searchBar.value = "";
        if (filter)
  ...
  doCommand: function(command)
  {
    switch(command)
    {
      case "cmd_selectAll":
        ...
        unifinderTree.view.selection.selectAll();
        return;
    }
  }
};
