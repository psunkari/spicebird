...
function taskViewUpdate(filter) {
    ...
    // calFilter looks pretty naive
    // but thankfully lets me give my own filter function.

    // Take care of the PropertyFilterBag as well as text filters.
    var taskFilter = function(aItem) {
      filterByText.mTextFilterField = "unifinder-todo-search-field";
      var textMatch = filterByText(aItem);

      var filterMatch = true;
      if(tree.mFilter.mPropertyFilterBag[filter])
        filterMatch = tree.mFilter.mPropertyFilterBag[filter](aItem);

      return textMatch && filterMatch;
    }

    tree.mFilter.propertyFilter = taskFilter;
