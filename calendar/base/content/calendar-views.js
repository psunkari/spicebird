...
function switchToView(aViewType) {
    ...
    ["calendar-previous-view-button",
     "calendar-next-view-button"].forEach(function(x) setupViewNode(x, "tooltiptext"));
...
function updateStyleSheetForViews(aCalendar) {
    ...
    if (!(aCalendar.id in ruleCache)) {
        ...
        ruleString = '.calendar-color-box[calendar-id="' + aCalendar.id + '"] .calendar-event-details-core {} ';
        ruleIndex = sheet.insertRule(ruleString, sheet.cssRules.length);
        ruleCache[aCalendar.id + "_inner"] = sheet.cssRules[ruleIndex];
    }
    ...
    ruleCache[aCalendar.id + "_inner"].style.backgroundColor = color;
}
...
let cal = cal || {};
cal.currentViewRange = "";
cal.currentWeekRange = "";
cal.navigationBar = {
    setDateRange: function setDateRange(aStartDate, aEndDate) {
        if (aStartDate) {
            ...
            if (aStartDate.nativeTime == aEndDate.nativeTime) {
                intervalRange = getDateFormatter().formatDate(aStartDate);
            } else {
                intervalRange = currentView().getRangeDescription();
                secondWeekNo = getWeekInfoService().getWeekTitle(aEndDate);
            }
            cal.currentViewRange = intervalRange;

            let weekRange = ""
            let weekRangeTooltip = "";
            if (secondWeekNo == firstWeekNo) {
                weekRange = calGetString("calendar", "singleShortCalendarWeek", [firstWeekNo]);
                weekRangeTooltip = calGetString("calendar", "singleLongCalendarWeek", [firstWeekNo]);
            } else {
                weekRange = calGetString("calendar", "severalShortCalendarWeeks", [firstWeekNo, secondWeekNo]);
                weekRangeTooltip = calGetString("calendar", "severalLongCalendarWeeks", [firstWeekNo, secondWeekNo]);
            }
            cal.currentWeekRange = weekRange;
            
            // If we have a navbar update it.
            let navbar = document.getElementById("calendar-navbar");
            if (navbar) {
              navbar.setAttribute("interval", intervalRange);
              navbar.setAttribute("weeks", weekRange);
            }
        }
    }
};
