...
function getCompositeCalendar() {
    if (gCurrentApplication && gCurrentApplication == "tasks")
        return getTasksCompositeCalendar();
...
function getTasksCompositeCalendar() {
    if (getTasksCompositeCalendar.mObject === undefined) {
        getTasksCompositeCalendar.mObject = Components.classes["@mozilla.org/calendar/calendar;1?type=composite"]
                                                 .createInstance(Components.interfaces.calICompositeCalendar);
        getTasksCompositeCalendar.mObject.prefPrefix = 'calendar-tasks';
        ...
        try {
            if (gCalendarStatusFeedback) {
                ...
                // our status observer.
                let chromeWindow = window.QueryInterface(Components.interfaces.nsIDOMChromeWindow);
                getTasksCompositeCalendar.mObject.setStatusObserver(gCalendarStatusFeedback, chromeWindow);
            }
        } catch (exc) { // catch errors in case we run in contexts without status feedback
        }
        */
    }
    return getTasksCompositeCalendar.mObject;
}
