...
calDavCalendar.prototype = {
    ...
    getItems: function caldav_getItems(aItemFilter, aCount, aRangeStart,
        ...
        // Return immediately with an error status, if the calendar is in error state!
        let currentStatus = this.getProperty("currentStatus");
        if (!Components.isSuccessCode(currentStatus)) {
          this.notifyOperationComplete(aListener, currentStatus,
                                       Components.interfaces.calIOperationListener.GET,
                                       null, "Calendar is in error state!");
        }
    ...
    checkDavResourceType: function caldav_checkDavResourceType(aChangeLogListener) {
        ...
        try {
          var httpchannel = cal.prepHttpChannel(this.makeUri(),
                                                queryXml,
                                                "text/xml; charset=utf-8",
                                                this);
        } catch(ex) {
          // Handle situations like an invalid protocol
