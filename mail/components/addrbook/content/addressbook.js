...
{
  gAbSearchInput = document.getElementById("peopleSearchInput");
...
function SetStatusText(total)
{
  ...
  try {
    ...
    if (gAbSearchInput && gAbSearchInput.value) {
      if (total == 0)
    ...
    if (gCurrentApplication == "contacts")
      gStatusText.setInterimStatus(statusText);
  }
  catch(ex) {
...
{
  var focusedElement    = abWhichPaneHasFocus();
