...
{
  toOpenWindowByType("collab:main", "chrome://collab/content/collab.xul");
}
...
function focusOnMail(tabNo, event)
{
  ...
  var topWindow = windowManagerInterface.getMostRecentWindow("collab:main");
  if (topWindow) {
    if (topWindow != window)
      topWindow.focus();
    else
      document.getElementById('tabcollab').selectTabByIndex(event, tabNo);
  }
  else {
    window.open("chrome://collab/content/collab.xul",
...
#else
  window.openDialog("chrome://collab/content/about.xul", "About", "centerscreen,chrome,resizable=no");
#endif
}
...
function getMostRecentMailWindow() {
  ...
  let win = wm.getMostRecentWindow("collab:main", true);

  // if we're lucky, this isn't a popup, and we can just return this
  if (win && win.document.documentElement.getAttribute("chromehidden")) {
    win = null;
    var windowList = wm.getEnumerator("collab:main", true);
...
#else
  var windowList = wm.getZOrderDOMWindowEnumerator("collab:main", true);
