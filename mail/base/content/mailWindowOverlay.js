...
function InitViewLayoutStyleMenu(event)
{
  ...
  var showMessagePane = document.getElementById("menu_showMessage");
  if (IsMessagePaneCollapsed())
    showMessagePane.removeAttribute("checked");
  else
    showMessagePane.setAttribute("checked", "true");
}
...
function InitViewHeadersMenu()
{
  ...
  document.getElementById("cmd_viewCompactHeader")
          .setAttribute("checked", headerchoice == dt.MicroHeaders);
...
function ToggleMessageTag(key, addKey)
{
  ...
  try {
    var addTodo = pref.getBoolPref("mailnews.tags." + key + ".todo");
    if (addTodo) {
      if (addKey) {
        var tagService = Components.classes["@mozilla.org/messenger/tagservice;1"]
                                   .getService(Components.interfaces.nsIMsgTagService);
        var tagName = tagService.getTagForKey(key);
        var description = tagName + ": ";
        for (var i = 0; i < gFolderDisplay.selectedMessageUris.length; ++i) {
          var msgHdr = messenger.msgHdrFromURI(gFolderDisplay.selectedMessageUris[i]);
          description = description + msgHdr.subject + "\n";
        }

        if (gFolderDisplay.selectedMessageUris.length == 1) {
          createTodoWithDialog(null, null, description, null);
        } else {
          var todo = createTodo();
          todo.title = tagName;
...
{
  /*
   * Spicebird does not have a button bar in headers
   *
...
{
  // On Spicebird we don't currently handle opening
  // more than one collab windows!
  return;

  if (folderURI) {
    window.openDialog("chrome://collab/content/", "_blank",
...
function MsgOpenNewTabForFolder(aBackground)
{
  ...
  let selectedFolders = gFolderTreeView.getSelectedFolders();
  for (let i = 0; i < selectedFolders.length; i++) {
    document.getElementById("tabcollab").openTab("folder",
...
function MsgOpenSelectedMessages()
{
  ...
  // This is somewhat evil. If we're in a 3pane window, we'd have a tabcollab
  // element and would pass it in here, ensuring that if we open tabs, we use
  // this tabcollab to open them. If we aren't, then we wouldn't, so
...
function MsgDisplayMessageInFolderTab(aMsgHdr)
{
  ...
  let folderTab = tabcollab.getTabInfoForCurrentOrFirstModeInstance(
                      tabcollab.tabModes["folder"]);
  let folderDisplay = folderTab.folderDisplay;
  let folder = aMsgHdr.folder;

  // XXX Yuck. We really need to have the tabcollab be able to handle an extra
  ...
  // Switch to the tab
  tabcollab.switchToTab(folderTab);
...
function MsgViewCompactHeaders()
{
  gPrefBranch.setIntPref("mail.show_headers", 0);
  ReloadMessage();
}

function MsgHeaderLessDetails()
{
  gPrefBranch.setIntPref("mail.show_headers",
                     gPrefBranch.getIntPref("mail.show_headers") <= 1? 0 : 1);
  ReloadMessage();
}

function MsgHeaderMoreDetails()
{
  gPrefBranch.setIntPref("mail.show_headers",
                     gPrefBranch.getIntPref("mail.show_headers") >= 1? 2 : 1);
  ReloadMessage();
}
...
var gMessageNotificationBar =
{
  ...
  {
    if (!gSynovelMessengerBundle)
      gSynovelMessengerBundle = document.getElementById("bundle_synovel_messenger");
    ...
    document.getElementById('loadAuthorRemoteContent').label = 
      gSynovelMessengerBundle.getFormattedString('alwaysLoadRemoteContentForSender1',
  ...
  {
    if (!this.mMsgNotificationBar)
      this.mMsgNotificationBar = document.getElementById('msgNotificationBar');
...
function OnMsgLoaded(aUrl)
{
  ...
  if (msgHdr && !msgHdr.isRead && markReadAutoMode)
  {
    ...
    if (markReadOnADelay && wintype == "collab:main") // 3-pane window
    {
      ClearPendingReadTimer();
