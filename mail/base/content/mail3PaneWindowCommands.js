...
var ThreadPaneController =
{
  ...
  onEvent: function(event)
  {
    if (event == 'select' || event == 'focus')
      UpdateDeleteCommand();

    else if (event == 'blur')
      // on blur events set the menu item text back to the normal value
      goSetMenuValue('cmd_delete', 'valueDefault');
  }
};

// We need the default controller to keep these
// commands available across all applications
var MailnewsGlobalController = 
{
  supportsCommand: function(command)
  {
    switch (command)
  ...
  {
    goUpdateCommand('button_getNewMessages');
  ...
  isCommandEnabled: function(command)
  {
    ...
    switch ( command )
    {
      ...
      case "cmd_applyFiltersToSelection":
        ...
        if (numSelected > 0 && (gCurrentApplication == "folder" || gCurrentApplication == "message"))
        {
      ...
      case "cmd_goStartPage":
        return document.getElementById("tabcollab").selectedTab.mode.name == "folder" &&
      ...
      case "cmd_findPrevious":
        ...
        if (document.getElementById("tabcollab").selectedTab.mode.name == "message")
          return true;
      ...
      case "cmd_msgSubscribe":
        return IsMessageSubscribeEnabled();
      case "cmd_properties":
...
function SetupCommandUpdateHandlers()
{
  ...
  if (threadTree)
    threadTree.controllers.appendController(ThreadPaneController);
}
...
function IsMessageSubscribeEnabled()
{
  ...
  for (var i = 0; i < allServers.Count(); i++) {
    ...
    if (currentServer.type == "nntp" ||
        currentServer.type == "imap" || currentServer.type == "rss")
      return true;
  }
...
function IsPropertiesEnabled(command)
{
  ...
  if (!folders.length || IsFakeAccount())
    return false;
  var folder = folders[0];
