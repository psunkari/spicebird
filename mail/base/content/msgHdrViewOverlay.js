...
var gShowCompactHeaderView = false;
...
var gCompactHeaderList =  [ {name:"subject"},
                            {name:"from", useToggle:true, outputFunction:OutputEmailAddresses},
                            {name:"date"},
                            {name:"tags"} ];
...
function initializeCompactHeaderViewTables()
{
  var index;
  for (index = 0; index < gCompactHeaderList.length; index++)
  {
    var headerName = gCompactHeaderList[index].name;
    gCompactHeaderView[headerName] = new createHeaderEntry('compact', gCompactHeaderList[index]);
  }
}
...
function OnLoadMsgHeaderPane()
{
  ...
  initializeCompactHeaderViewTables();
...
function OnAddressBookDataChanged(aAction, aParentDir, aItem) {
  gEmailAddressHeaderNames.forEach(function (headerName) {
      ...
      if (headerName in gExpandedHeaderView || headerName in gCompactHeaderView) {
        ...
        headerEntry = gCompactHeaderView[headerName];
        if (headerEntry)
          headerEntry.enclosingBox.updateExtraAddressProcessing(aAction,
                                                                aParentDir,
                                                                aItem);
...
var messageHeaderSink = {
    ...
    {
      this.mSaveHdr = null;
      // every time we start to redisplay a message, check the headers pref.
      var showHeadersPref = pref.getIntPref("mail.show_headers");
      if (showHeadersPref == 0)
      {
        document.getElementById("compactHeaderView").removeAttribute("collapsed");
        document.getElementById("msgHeaderViewDeck").selectedIndex = 1;
        document.getElementById("expandedHeaderView").setAttribute("collapsed", true);
        document.getElementById("msgHeaderLessButton").setAttribute("disabled", true);
        document.getElementById("msgHeaderMoreButton").removeAttribute("disabled");
        gShowCompactHeaderView = true;
      }
      else
      {
        document.getElementById("expandedHeaderView").removeAttribute("collapsed");
        document.getElementById("msgHeaderViewDeck").selectedIndex = 0;
        document.getElementById("compactHeaderView").setAttribute("collapsed", true);
        gShowCompactHeaderView = false;

        if (showHeadersPref == 2)
        {
          document.getElementById("msgHeaderLessButton").removeAttribute("disabled");
          document.getElementById("msgHeaderMoreButton").setAttribute("disabled", true);
    ...
    onEndHeaders: function()
    {
      ...
      // trigger a reflow!
      ClearHeaderView(gShowCompactHeaderView? gCompactHeaderView : gExpandedHeaderView);

      EnsureSubjectValue(); // make sure there is a subject even if it's empty so we'll show the subject and the twisty

      // Only update the currently selected view (and if not already done)
      let hdrViewDeck = document.getElementById('msgHeaderViewDeck');
      if (hdrViewDeck.selectedIndex == 0 && !gBuiltExpandedView)
        UpdateExpandedMessageHeaders();
      else if(hdrViewDeck.selectedIndex == 1 && !gBuiltCompactView)
        UpdateCompactMessageHeaders();
...
function OnTagsChange()
{
  ...
  if (gBuiltCompactView)
  {
    var headerEntry = gCompactHeaderView.tags;
    if (headerEntry)
    {
      headerEntry.valid = ("tags" in currentHeaderData);
      if (headerEntry.valid)
...
{
  if (document.getElementById('msgHeaderViewDeck').selectedIndex != 1)
    return;

  syncGridColumnWidths(true);
  showHeaderView(gCompactHeaderView);
}
 ...
 */
function syncGridColumnWidths(isCompact) 
{
  let nameColumn = isCompact? document.getElementById('compactHeadersNameColumn'):
                              document.getElementById('expandedHeadersNameColumn');
  let nameColumn2 = isCompact? document.getElementById('compactHeaders2NameColumn'):
                               document.getElementById('expandedHeaders2NameColumn');
...
function UpdateCompactMessageHeaders() {
  ...
  for (headerName in currentHeaderData) {
    ...
    if (headerName in gCompactHeaderView)
      headerEntry = gCompactHeaderView[headerName];
