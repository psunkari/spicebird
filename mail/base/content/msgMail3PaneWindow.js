...
function OnLoadMessenger()
{
  ...
  if (tabmail)
  {
    ...
    tabmail.registerTabMonitor(GlodaToolbarHackTabMonitor);
    let tab = tabmail.openTab("folder",
                              {background: true,
                               messagePaneVisible: true});
    tab.folderDisplay.makeActive();
  }
...
function LoadPostAccountWizard()
{
  ...
  function completeStartup() {
    ...
    if (nsIShellService) {
      ...
      var defaultAccount;
      try {
        shellService = Components.classes["@synovel.com/collab/shell-service;1"].getService(nsIShellService);
...
function FindOther3PaneWindow()
{
  ...
  // on Linux
  let enumerator = windowMediator.getEnumerator("collab:main");
 ...
 */
function OnUnloadMessenger(event)
{
  ...
  /**
   ** XXX (Prasad): Persisting of tabs is currently disabled in Spicebird
   **/
...
function loadExtraTabs()
{
  if ("arguments" in window && window.arguments.length >= 2) {
    if (window.arguments[1] && "tabType" in window.arguments[1]) {
      document.getElementById('tabcollab').openTab(window.arguments[1].tabType, window.arguments[1].tabParams);
    }
  }
}
...
function UnloadPanes()
{
  ...
  gFolderTreeView.unload("messenger_folders.json");
...
function OpenMessageInNewTab(event)
{
  ...
  document.getElementById("tabcollab").openTab("message",
...
{
  let openMessageBehaviorVersion;
  try {
    openMessageBehaviorVersion = gPrefBranch.getIntPref(
