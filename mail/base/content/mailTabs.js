...
let mailTabType = {
  ...
  modes: {
    ...
    folder: {
      ...
      openTab: function(aTab, aArgs) {
        ...
        //  "folder" tab.)
        let modelTab = document.getElementById("tabcollab")
                         .getTabInfoForCurrentOrFirstModeInstance(aTab.mode);

        // Make sure we are not returned the tab that we are creating
        if (modelTab == aTab)
          modelTab = null;
      ...
      shouldSwitchTo: function(aArgs) {
        // Open a new tab if "switchToExisting" is not defined or is not true.
        let switchToExisting = aArgs && aArgs.switchToExisting;
        if (!switchToExisting)
          return -1;
  
        let tabcollab = document.getElementById("tabcollab");
        let tabInfo = tabcollab.getTabInfoForCurrentOrFirstModeInstance(this.modes.folder);
  
        // If we dont have a tab, open new
        if (!tabInfo)
          return -1;
  
        return tabcollab.tabInfo.indexOf(tabInfo);
      }
    },
    /**
  ...
  showTab: function(aTab) {
    ...
    let focusedElement = aTab.focusedElement;
    if (typeof focusedElement == 'undefined' || !focusedElement) {
      if (aTab.mode.type == "folder")
        focusedElement = document.getElementById("folderTree");
      else
        focusedElement = document.getElementById("messagePane");
    }
    if (focusedElement)
  ...
  //__proto__: "DefaultMailnewsController" in window && window.DefaultMailnewsController ||
  ...
  supportsCommand: function mailTabType_supportsCommand(aCommand, aTab) {
    switch (aCommand) {
      ...
      default:
        return DefaultMailnewsController.supportsCommand(aCommand);
    }
  },
  ...
  isCommandEnabled: function mailTabType_isCommandEnabled(aCommand, aTab) {
    switch (aCommand) {
      ...
      default:
        return DefaultMailnewsController.isCommandEnabled(aCommand);
    }
  },
  ...
  doCommand: function mailTabType_doCommand(aCommand, aTab) {
    ...
    DefaultMailnewsController.doCommand(aCommand, aTab);
  }
};
