...
var MailUtils =
{
  ...
  displayMessages: function MailUtils_displayMessages(aMsgHdrs,
    ...
    if (openMessageBehavior == MC.OpenMessageBehavior.NEW_WINDOW) {
      ...
      if (!aTabmail) {
        ...
        mail3PaneWindow = windowMediator.getMostRecentWindow("collab:main");
        if (mail3PaneWindow)
          aTabmail = mail3PaneWindow.document.getElementById("tabcollab");
      }
  ...
  displayMessageInFolderTab: function MailUtils_displayMessageInFolderTab(
    ...
    let windowMediator = Cc["@mozilla.org/appshell/window-mediator;1"]
                           .getService(Ci.nsIWindowMediator);
    let mail3PaneWindow = windowMediator.getMostRecentWindow("collab:main");
    if (mail3PaneWindow) {
